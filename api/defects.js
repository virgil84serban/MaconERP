...
module.exports = async function handler(request, response) {
  ...
  try {
    ...
    if (request.method === "GET") {
      const result = await listDefects(context);
    ...
    if (request.method === "POST") {
      const result = await createDefect(payload);
      return response.status(result.ok ? 200 : 400).json(result);
    }

    if (request.method === "PUT") {
      const result = await updateDefect(payload);
      return response.status(result.ok ? 200 : 400).json(result);
    }

    if (request.method === "DELETE") {
      const result = await deleteDefect(payload);
      return response.status(result.ok ? 200 : 400).json(result);
    }
