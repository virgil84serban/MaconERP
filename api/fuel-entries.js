...
module.exports = async function handler(request, response) {
  ...
  try {
    ...
    if (request.method === "GET") {
      const result = await listFuelEntries(context);
    ...
    if (request.method === "POST") {
      const result = await createFuelEntry(payload);
      return response.status(result.ok ? 200 : 400).json(result);
    }

    if (request.method === "PUT") {
      const result = await updateFuelEntry(payload);
      return response.status(result.ok ? 200 : 400).json(result);
    }

    if (request.method === "DELETE") {
      const result = await deleteFuelEntry(payload);
      return response.status(result.ok ? 200 : 400).json(result);
    }
