...
module.exports = async function handler(request, response) {
  ...
  try {
    ...
    if (request.method === "GET") {
      const result = await listRevisions(context);
    ...
    if (request.method === "POST") {
      const result = await createRevision(payload);
      return response.status(result.ok ? 200 : 400).json(result);
    }

    if (request.method === "PUT") {
      const result = await updateRevision(payload);
      return response.status(result.ok ? 200 : 400).json(result);
    }

    if (request.method === "DELETE") {
      const result = await deleteRevision(payload);
      return response.status(result.ok ? 200 : 400).json(result);
    }
